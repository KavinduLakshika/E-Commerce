...
const createUser = async (req, res) => {
    upload(req, res, async function (err) {
        ...
        try {
            ...
            let userImg = null;
            if (req.file) {
                userImg = `${req.protocol}://${req.get('host')}/uploads/users/${req.file.filename}`;
            }
    ...
    upload(req, res, async function (err) {
        ...
        try {
            ...
            if (req.file) {
                ...
                if (userImg) {
                    const oldImagePath = path.join(__dirname, '..', 'uploads', 'users', path.basename(userImg));
...
const userLogin = async (req, res) => {
    try {
        ...
        // Verify password
        const passwordMatch = await bcrypt.compare(userPassword, user.userPw);
        if (!passwordMatch) {
